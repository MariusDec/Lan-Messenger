...
QString Helper::getUuid() {
  QString Uuid = QUuid::createUuid().toString();
  Uuid = Uuid.remove("{").remove("}").remove("-");
  return Uuid;
}

QString Helper::getLogonName() {
...
QString Helper::getHostName() { return QHostInfo::localHostName(); }
...
int Helper::compareVersions(const QString &version1, const QString &version2) {
  ...
  //	Assuming that the version is in x.x.x format, we only need to iterate 3
  // times
