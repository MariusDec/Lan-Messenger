...
void FileDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
  QVariant variant = QVariant::fromValue(index.data());
  if(variant.canConvert<FileView>()) {
      FileView fileView = variant.value<FileView>();
...
void FileModel::saveData(const QString &filePath, const FileView *transfer) {
  if(transferList.isEmpty() && !transfer)
    return;
