...
bool QxtToolTipPrivate::eventFilter(QObject* object, QEvent* event)
{
    ...
    case QEvent::Wheel:
    {
        ...
        QLabel *label = tooltips.value(widget).first->findChild<QLabel *>(QStringLiteral("labelImage"));
        if (label)
            label->movie ()->stop ();
        break;
    }
...
void QxtToolTip::setToolTip(QWidget *parent, QWidget *tooltip, const QRect &rect) {
    ...
    else
    {
        ...
        if (!QxtToolTipPrivate::instance()->tooltips.contains(parent))
            ...
            QPointer<QWidget> tooltipQidget = QxtToolTipPrivate::instance()->tooltips[parent].first;
            QxtToolTipPrivate::instance()->tooltips.remove(parent);
            if (tooltipQidget.data())
                tooltipQidget.data()->deleteLater();
        }
    }
}

void QxtToolTip::removeToolTip(QWidget *parent){
    QPointer<QWidget> tooltipQidget = QxtToolTipPrivate::instance()->tooltips[parent].first;
    QxtToolTipPrivate::instance()->tooltips.remove(parent);
    if (tooltipQidget.data())
        tooltipQidget.data()->deleteLater();
