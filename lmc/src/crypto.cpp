...
QByteArray lmcCrypto::encrypt(QString* lpszUserId, QByteArray& clearData) {
    ...
    if(outBuffer == NULL) {
        LoggerManager::getInstance().writeError("Buffer not allocated");
        return QByteArray();
    }
    int foutLen = 0;
    ...
    LoggerManager::getInstance().writeError("Message encryption failed");
    return QByteArray();
}
...
QByteArray lmcCrypto::decrypt(QString* lpszUserId, QByteArray& cipherData) {
    ...
    LoggerManager::getInstance().writeError("Message decryption failed");
    return QByteArray();
}
